...
syms OF_pc_sym
m_pc = 0;       % Seleccionar el tipo de mezcla en la precámara: 1 para rica (exceso de H2), 0 para pobre (exceso de O2)
...
Pc = linspace(1e5, 50e6, 50);
 ...
 %OF =  zeros(length(OF_cc), length(Pc));
 %Isp_Vol = zeros(length(OF_cc), length(Pc));
 %DV = zeros(length(OF_cc), length(Pc));
 A_g = zeros(length(OF_cc), length(Pc));
...
%%%%%%%%%%%
    ...
    %%  OPTIMIZACIÓN (APARTADO 3) - IMPULSO VOLUMÉTRICO
    %rho_m = (1 + OF) / ( (OF / rho_ox) + (1 / rho_f));
    rho_m = (OF * rho_ox + rho_f)/(1 + OF);
    
    Isp_Vol = Isp * rho_m;

    [i, j] = find(ismember(Isp_Vol, max(Isp_Vol(:))))   
    OF_cc(i)
    Pc(j)

    %%  OPTIMIZACIÓN (APARTADO 4) - DELTA V
    DV = Isp * log((1 + K_m)/(r + K_m));
    
    %% DIMENSIONALIZAR
    
    A_g(i,j) = E / (C_E(i,j) .* Pc(j)); 
...
%%%%%%%%%%%%%%%%%%%%%% 3D PLOT Isp / (1+K_m)
figure()
surf(vo,(vx(:)/1e5),Isp_Km)
xlabel('O/F (CC)')
ylabel('Pc [bar]')
zlabel('Isp / { 1+K_m } [m/s]')
colorbar
axis tight

%%%%%%%%%%%%%%%%%%%%%% 3D PLOT Isp Volumetrico
figure()
surf(vo,(vx(:)/1e5),Isp_Vol)
xlabel('O/F (CC)')
ylabel('Pc [bar]')
zlabel('Isp_{vol} [N.s/{m^3}]')
colorbar
axis tight
