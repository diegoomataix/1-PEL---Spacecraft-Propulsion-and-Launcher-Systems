...
x =  linspace(0, 2, 20);
%x =  linspace(4, 20, 10);
Pc = linspace(1e5, 100e6, 20);
%Pc = linspace(20e6, 40e6, 10);
...
% incógnita y resolvemos
    ...
    elseif mt_m_v(i,j) > 1
        ...
        mt_m_v(i,j) = mt_m_v(i,j);
    end
    
% Gastos másicos
    m_o_pc = double(mt_m_v(i, j)) * (OF_pc / (OF_pc + 1 )); %  gasto masico oxidante pre- camara / m total
    m_f_pc = double(mt_m_v(i, j)) / (OF_pc + 1);            %  gasto masico fuel pre- camara / m total
...
%     x_cc(i,j) = (m_o_c/m_f_c)*M_o2/M_h2;
%    z = x_cc(i,j)*M_h2/M_o2;
     A = m_f_c / 2;
%     A = 1;
     B = m_o_c / 32;
%     B = z;
    ...
    x_cc(i,j) = (m_o_c/m_f_c) *M_o2 / M_h2; % O/F CC
